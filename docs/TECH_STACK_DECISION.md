...
- “打开笔记”动作统一走同一条前端选择通路：负责工作区切换、目录定位、未保存保护与编辑器焦点恢复，避免 search / graph / media / links 各自维护一套跳转逻辑。
- 搜索结果跳转继续复用同一通路，但额外挂一个轻量待恢复选区，待文档加载完成后在标题或正文中选中首个命中位置。
- 同步 UX 继续基于现有 `SyncStatusResponse` 数据结构增强，不新增后端协议；前端通过状态摘要卡、推荐动作和快照卡片把已有状态转成用户可读信息。
